...
struct PersistenceController {
    ...
    static var preview: PersistenceController = {
        ...
        var loc: Location = Location(context: viewContext)
        loc.name = "Bathroom"
        ...
        for _ in 0..<10 {
            ...
            newWorkItem.dueDate = Date()
            newWorkItem.locations = Set<Location>()
            newWorkItem.locations?.insert(loc)
        }
    ...
    init(inMemory: Bool = false) {
        container = NSPersistentContainer(name: "Homehut")
        if inMemory {
